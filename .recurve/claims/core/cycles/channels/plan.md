...
| --- | --- | --- | --- | --- |
| MC-19 | core | headline | missing-surface | `mc-19.sh` |

## Smallest fixes (the SCULPT scope — keep it minimal, type-driven)

- **MC-19** — auths-mcp channel open --seller --capacity --rail records the funded reservation the gateway meters against; channel close settles the netted total in one rail action and emits a settlement record; rail legs env-gated with a stated skip.

## What gets stronger (the REBUILD payoff)

- **MC-19** unlocks: (state what gets stronger when this closes)
...
```
  ...
  ● MC-19       GREEN    closed               channel open/close present with env-gated rail legs
