...
| --- | --- | --- | --- | --- |
| MC-10 | core | feature | missing-surface | `mc-10.sh` |

## Smallest fixes (the SCULPT scope — keep it minimal, type-driven)

- **MC-10** — Add example_call (tools/call arguments with amount_atomic derived from price_cents) to GET /api/v1/endpoints/<slug> and the MCP get_integration tool.

## What gets stronger (the REBUILD payoff)

- **MC-10** unlocks: (state what gets stronger when this closes)
...
```
  ...
  ● MC-10       GREEN    closed               example_call served by API and MCP directory
