...
| --- | --- | --- | --- | --- |
| MC-9 | core | feature | wire-mismatch | `mc-9.sh` |

## Smallest fixes (the SCULPT scope — keep it minimal, type-driven)

- **MC-9** — parseListingInput refuses endpointValue containing the wrap launcher, teaching the bare downstream command; keep the demo-echo seed raw.

## What gets stronger (the REBUILD payoff)

- **MC-9** unlocks: (state what gets stronger when this closes)
...
```
  ...
  ● MC-9        GREEN    closed               raw-downstream convention enforced and seeded
