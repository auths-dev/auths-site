...
| --- | --- | --- | --- | --- |
| MC-15 | core | feature | missing-surface | `mc-15.sh` |

## Smallest fixes (the SCULPT scope — keep it minimal, type-driven)

- **MC-15** — New route: authenticate the presentation, return the caller-owned listings with status, fail_reason, verified_at, live_proven_at.

## What gets stronger (the REBUILD payoff)

- **MC-15** unlocks: (state what gets stronger when this closes)
...
```
  ...
  ● MC-15       GREEN    closed               agents can read their own listings
