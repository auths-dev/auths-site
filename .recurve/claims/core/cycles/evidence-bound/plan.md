# Sculpting cycle: evidence-bound
...
| --- | --- | --- | --- | --- |
| MC-8 | core | feature | missing-surface | `mc-8.sh` |

## Smallest fixes (the SCULPT scope — keep it minimal, type-driven)

- **MC-8** — Bound the Auths-Presentation evidence at 64 KB before any parse/verify, denial code evidence-too-large, in the market auth lib.

## What gets stronger (the REBUILD payoff)

- **MC-8** unlocks: (state what gets stronger when this closes)
...
```
  ...
  ● MC-8        GREEN    closed               oversized evidence refused with typed 401
