...
Link your GitHub, npm, or PyPI accounts so the world knows exactly which DID belongs to you. A hosted public registry for publishing and discovering attestations is coming soon — self-hosted registries work today.
...
**The registry (optional, coming soon)** Auths functions perfectly completely offline. The upcoming hosted registry adds an index of the ecosystem for discovery — letting consumers ask things like, *"Show me the verified public keys for the GitHub user @torvalds."* Self-hosting via `--registry <url>` works today.

---

## The Auths Lifecycle

1. **Create your permanent identity** When you run `auths init`, Auths generates a cryptographic inception event. This establishes your permanent identifier (`did:keri:...`). You don't rely on a central Certificate Authority (CA) or OpenID provider; you are your own root of trust.
...
3. **Sign and publish** When you build a release, your local device key signs the artifact's hash. You can log the signature to Sigstore's public transparency log with one flag, or publish the attestation to a registry.
...
<div className="mt-2 mb-4 text-zinc-300">
You can sign anything that can be hashed. Currently, <code>auths</code> seamlessly supports Git commits and any artifact file — release tarballs, npm package tarballs, crates, and raw binaries. Container-image signing is on the roadmap.
</div>
</details>
