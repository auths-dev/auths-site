...
Preview it at [auths.dev/registry](/registry). The network activity feed shows identity registrations, artifact attestations, and key lifecycle events. Each entry links to a DID. Each DID links to platform claims, device keys, and signed artifacts.

Underneath, every operation is append-only, Merkle-tree-backed, and independently verifiable. The hosted public registry is coming soon — self-hosted registries work today.
