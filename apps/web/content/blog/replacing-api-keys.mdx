...
Your DID is a public identifier. You can print it on a billboard. Knowing it doesn't let anyone impersonate you, because authentication requires a signature from the private key that's locked in your device's keychain (macOS Keychain, Windows Credential Manager, or a hardware token).
...
$ auths artifact sign package.tar.gz
$ auths artifact publish package.tar.gz --package npm:my-package --registry <registry-url>
$ auths artifact publish package.tar.gz --package pypi:my-package --registry <registry-url>
...
Pre-rotation solves this. When you create your identity, you also commit to what your *next* key will be (via a hash commitment). When you rotate, the new key matches the commitment, proving continuity. Your DID stays the same. Your old signatures remain valid. Your reputation carries forward.

```bash
$ auths id rotate
[OK] Key rotated. DID unchanged: <root>
...
```
[REVOKE] <root> revoked did:key:z6Mkjj...    #35
```
...
## What this looks like on a registry

A registry here is a log you can host yourself. Its activity feed shows identity registrations, artifact attestations, and key lifecycle events. Each entry links to an identity; each identity links to platform claims, device keys, and signed artifacts.

Underneath, every operation is append-only, Merkle-tree-backed, and independently verifiable. Self-hosted registries work today.
