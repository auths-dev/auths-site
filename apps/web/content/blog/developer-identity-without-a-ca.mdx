...
description: "How key event logs handle algorithm migration, key rotation, and post-quantum readiness compared to CA-dependent systems like Sigstore — and the honest tradeoffs between both approaches."
date: "2026-03-07"
---

# Developer Identity Without a Certificate Authority

> **Historical implementation note:** This article describes the experimental `auths-dev/auths` predecessor and its `0.1.x` commands. Those commands are preserved as project history, not current installation guidance. For the current Auths implementation, SDK work, and release status, use [auths-dev/auths-proof](https://github.com/auths-dev/auths-proof).
...
Auths takes a different position: your identity is *derived* from cryptographic material you control. Verification checks the key that signed, not a certificate chain — there's no trust root to refresh and no OIDC login flow. Your identity is a key event log (KEL) — a hash-chained, append-only, self-certifying sequence of key lifecycle events stored in your own Git repository.

## What a Key Event Log actually looks like

When you run `auths init`, the system generates two keypairs (P-256 by default — hardware-backed where the platform supports it) and creates an inception event:
...
The inception event is then hashed to produce a Self-Addressing Identifier (SAID), which becomes both the event's `d` field and the identity's permanent prefix (`i`). Your identifier is a 44-character string derived purely from the content of your inception event — it's not assigned by any authority.
...
Pre-rotation eliminates this window. When you rotate, the system verifies that your new key matches the commitment made in the *previous* event:
...
Auths stores the KEL as a chain of Git commits under `refs/did/keri/<prefix>/kel`. Each commit contains a single `event.json`. The Git commit chain mirrors the event chain:
...
- **Replication anywhere Git goes** — clone, push, pull. No special infrastructure.
...
That's a fair critique of Sigstore — and it's the exact problem an identity you carry with you resolves. Your identifier isn't an email address someone else controls. It's derived from your own key material.
...
We should be honest: this migration path exists in theory but hasn't been implemented yet. The trait needs to be generalized, the key-encoding scheme needs new codes for post-quantum key types, and the commitment scheme needs to be upgraded to a post-quantum hash (though Blake3 is already considered quantum-resistant for preimage attacks). This is real engineering work, not a flip of a switch. But the architecture doesn't need to change. That matters.
...
**Transparency log auditability.** Rekor provides a public, append-only, globally consistent log that anyone can monitor for unauthorized signing events. Auths stores each identity's log in its own Git repo — which also means there's no single place to watch; you have to go find the log to audit it.
...
**Key lifecycle durability.** Auths identifiers survive key rotations, device changes, and algorithm migrations. Your identifier is permanent — it's derived from your inception event, not from any particular key. Sigstore's identity is your email address or CI identity, which is durable in a different sense (tied to your account) but not something you cryptographically own.
...
Software signing infrastructure is in a genuinely difficult design space. You're balancing ease of adoption against depth of security guarantees, the convenience of central services against resilience that doesn't depend on them, and present-day pragmatism against future-proofing.
...
The code referenced in this post is open source at [github.com/auths-dev/auths](https://github.com/auths-dev/auths).
