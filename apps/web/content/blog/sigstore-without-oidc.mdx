...
This post explains what happened, why it matters, and what it means for the relationship between an identity you control and the supply chain security infrastructure the industry is converging on.

## What Auths is (30-second version)

Auths is a code signing tool that gives developers a cryptographic identity without relying on any third party. You run `auths init`, a P-256 key goes into your OS keychain, a key event log — an append-only, self-certifying history of your key — is created in Git, and your commits are signed. No accounts, no servers, no ceremony.
...
We skipped Fulcio and OIDC entirely. Auths has its own identity model (key event logs), its own keys (P-256 in your OS keychain), and its own attestation format. We only needed Rekor for one thing: a public, append-only record that the signing event occurred.
...
That's a P-256 public key. The same key that's in the developer's OS keychain and referenced in their key event log. No certificate wrapping it. No OIDC claim binding it to an email address. Just a key and a signature.
