...
That DID — `<root>` in the output above — is now your identity everywhere. Query the registry with it:

```bash
$ curl -s https://auths.dev/v1/identities/<root>
{
  "status": "active",
  "did": "<root>",
  "public_keys": [...],
...
```
[NAMESPACE] <maintainer> claimed cargo:xz-utils          #20
[ORG]       <maintainer> added <jia-tan>                 #25
[DEVICE]    <jia-tan> bound did:key:z6Mk...              #28
[ATTEST]    <jia-tan> signed cargo:xz-utils 5.6.0        #31
[ATTEST]    <jia-tan> signed cargo:xz-utils 5.6.1        #32
```
...
```
[REVOKE] <maintainer> revoked <jia-tan>    #35
```
...
```
<root> → npm, PyPI, Docker, GitHub, any registry
```
...
- **Rotation doesn't break history.** Pre-rotation — committing to the hash of your next key in advance — means you can rotate to a new key without invalidating past signatures. The DID stays the same. The verification chain is unbroken.
...
This isn't a crypto network. There's no token, no consensus mechanism, no gas fees. It's a key event log — an append-only, self-certifying history of your key — with a witness network for consistency. If you want the deep technical comparison, we covered [how key event logs work](/blog/developer-identity-without-a-ca) in a previous post.

This isn't theoretical. The CLI is shipped, the SDKs exist for Python and Node, and the hosted registry is on the way.
...
Preview the registry: [auths.dev/registry](/registry)
