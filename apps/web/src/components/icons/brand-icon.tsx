/**
 * Unified brand icon component for ecosystems and platforms.
 *
 * Consolidates EcosystemIcon and PlatformIcon into a single lookup.
 * Uses Simple Icons for package ecosystems and inline SVGs for forges.
 * Falls back to `null` for unknown names.
 *
 * @param name      - Lowercase identifier (e.g. `"npm"`, `"github"`, `"cargo"`).
 * @param size      - Icon width/height in pixels (default 20).
 * @param className - Optional CSS class for colour/layout overrides.
 *
 * @example
 * <BrandIcon name="npm" size={24} className="text-red-500" />
...
// ---------------------------------------------------------------------------
// Inline SVG icons for platforms not in Simple Icons
// ---------------------------------------------------------------------------

function GitHubIcon({ size = 20, className }: IconProps) {
  return (
...
function GitLabIcon({ size = 20, className }: IconProps) {
  return (
...
function BitbucketIcon({ size = 20, className }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
...
function GiteaIcon({ size = 20, className }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
...
function RadicleIcon({ size = 20, className }: IconProps) {
  return (
    <img
      src="/images/radicle_logo.svg"
      alt="Radicle"
      width={size}
      height={size}
      className={className}
...
const ICONS: Record<string, ComponentType<IconProps>> = {
  // Package ecosystems (Simple Icons)
  npm: SiNpm,
  pypi: SiPypi,
  cargo: SiRust,
  docker: SiDocker,
...
export function EcosystemIcon({ ecosystem, size = 20, className }: { ecosystem: string; size?: number; className?: string }) {
  return <BrandIcon name={ecosystem} size={size} className={className} />;
}

export function PlatformIcon({ platform, size = 20, className }: { platform: string; size?: number; className?: string }) {
