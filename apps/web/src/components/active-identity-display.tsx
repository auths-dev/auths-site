/**
 * Renders the full profile of an active identity resolved by DID.
 *
 * Shows the DID, verified platform claims, public keys, and any signed
 * artifacts associated with the identity.
 *
 * @param data - The `ActiveIdentity` response from `GET /v1/identities/{did}`.
 *
 * @example
 * <ActiveIdentityDisplay data={activeIdentity} />
 */
...
import type { ActiveIdentity } from '@/lib/api/registry';
import { ArtifactResults } from '@/components/artifact-results';

export function ActiveIdentityDisplay({
  data,
  fromQuery,
}: {
  data: ActiveIdentity;
    ...
    >
      ...
      {data.artifacts.length > 0 && (
        <div>
          ...
          <ArtifactResults entries={data.artifacts} />
        </div>
      )}
